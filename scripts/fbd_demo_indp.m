...
T2ae = zeros(rank(X2n)+1,rank(X2n));

for ii = 1:5
    perms = randperm(size(E2,1));
    Eperm = E2(perms,:);
    X2perm = X2n + Eperm;
    X2he = Z2*pinv(Z2)*X2perm;
    T2he = X2he*V2;
    [T2ue, ~, ~] = uniquetol(T2he,1e-6, 'ByRows', true, 'PreserveRange', true);
    T2ae = T2ae + T2ue;
end
...
S2il = chol(pinv(cov(T2su)),'lower');
S12l = (N^(-1))*(T1su*R)'*T2su;
...
for ii = 1:n_perms
    ...
    T2pe = T2ae.*(1/5) - T2nu;
    S2pl = chol(pinv(cov(T2pe)),'lower');
    S12p = (N^(-1))*(T1su*R)'*T2pe;

    Fp(ii) = norm(S1il*S12p*S2pl','fro')^2;
end
