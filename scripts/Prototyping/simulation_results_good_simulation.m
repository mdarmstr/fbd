...
Fd = (2*pi * det(pinv(pinv((Er'*Er)/N) + pinv((Ep'*Ep)/N))))^(-size(Er,2)/2)*norm(J*T1u*(P - R)*Lobs,'fro')^2;
...
%[Qr,~] = qr(T1u*P*Lobs,'econ'); [Qp,~] = qr(T1u*R*Lobs,'econ');
%Fd = norm(Qr - Qp,'fro');
...
for ii = 1:n_perms
    ...
    Fp(ii) = (2*pi * det(pinv(pinv((Er'*Er)/N) + pinv((Ep'*Ep)/N))))^(-size(Erp,2)/2)*norm(J*T1up*(Pp - Rp)*L,'fro')^2;
    %[Qr,~] = qr(T1up*Pp*L,'econ'); [Qp,~] = qr(T1up*Rp*L,'econ');
    %Fp(ii) = norm(Qr - Qp,'fro');
