...
% E    = X1 - Xhat;
% 
% X1X2 = (X1 + X1ne)*V1*R*V2';
% [~,~,V2t] = svds(X1X2,rank(X2n));
% 
% Fd = norm(V2t - V2,'fro');
...
for ii = 1:n_perms
    ...
    [Rp,Pp,T1p,~,~,~] = diasmetic_rotations(Tpm,T2o,F1,F2);
    %[~,~,Vp] = svds((X1perm + Eperm)*Vpm*Rp*V2',rank(X2n));
    [Qp,~] = qr(T1p,'econ');


    Fp(ii) = trace(Rp'*Qp'*Q2)^2;
