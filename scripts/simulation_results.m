...
Fd = 0.5*norm(T1u*(R - P)*Lobs,'fro')^2;

%Fd = KL_divergence(Er,Ep);
...
for ii = 1:n_perms
    ...
    Tpm = (X1perm) * Vpm;
    [Rp,Pp,T1up,~,~] = diasmetic_rotations(Tpm,T2o,F1,F2);
 
    Fp(ii) = 0.5*norm(T1up*(Rp - Pp)*Lobs,'fro')^2;
end
