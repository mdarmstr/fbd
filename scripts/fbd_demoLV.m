...
[T2u, ~, ~] = uniquetol(T2o,1e-6, 'ByRows', true, 'PreserveRange', true);
...
t11 = chol(pinv(cov(T1u*R)),"lower");
t22 = chol(pinv(cov(T2u)),"lower");
t12 = (N^-1)*(R'*T1u'*T2u);

Fd = norm(t11*t12*t22','fro')^2;
...
for ii = 1:n_perms
    ...
    Bperm = pD1*Xperm;
    X1perm = Z1*Bperm;
    %[~,~,V] = svds(X2perm,rank(X2perm));
    
    Tpm = X1perm * V1;
    [Tpu, ~, ~] = uniquetol(Tpm,1e-6, 'ByRows', true, 'PreserveRange', true);

    t11 = chol(pinv(cov(Tpu*R)),"lower");
    t12 = (N^-1)*(R'*Tpu'*T2u);
    Fp(ii) = norm(t11*t12*t22','fro')^2;
