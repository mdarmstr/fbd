...
ylabel('Power');
legend('$p$ = $\alpha$','Null', 'Alternative', 'Location', 'southeast');
...
%% ===================== OVERLAY: Null distributions =====================
figure('Name','Overlay of null distributions');
hold on; box on;

% Common bin edges so the histograms are comparable
allF = [Fp_pos_plt(:); Fp_neg_plt(:)];
edges = linspace(min(allF), max(allF), 150);

% Positive-case null
histogram(Fp_pos_plt, edges, ...
    'Normalization','pdf', ...
    'FaceAlpha',0.4, ...
    'EdgeColor','none');

% Negative-case null
histogram(Fp_neg_plt, edges, ...
    'Normalization','pdf', ...
    'FaceAlpha',0.4, ...
    'EdgeColor','none');
...
title('Overlay of empirical null distributions');

legend({'Positive-case null','Negative-case null'}, 'Location','best');
...
[R,P,T1u,Er,Ep,T2u,S1u,V1u,S2u,V2u] = diasmetic_rotations(T1o,T2o,F1,F2);
...
%sxx = chol(pinv(cov(Er)),'lower');
%syx = (Er'*T2u*Lobs);
%syy = chol(pinv(cov(T2u)),'lower');


%Fd = 2*trace(R'*T1u'*T2u)^2 - trace(T1u'*T2u)^2;
%Fd = trace(Er'*T2u);
%Fd = norm(sxx*syx*syy','fro');
%Fd = norm(Er-T2u,'fro')^2;
%Fd = trace(Er'*T2u);
%Fd = 2*trace(Er'*T2u) - norm(T2u,'fro')^2;
Fd = trace(Er'*T2u);
Fp = zeros([1,n_perms]);
...
for ii = 1:n_perms
    ...
    %[I,K,L] = svds(X1perm,rank(X1perm));
    Tpm = X1perm*V1;
    %Tpm = X1perm  * V1;

    %[Rp,Pp,Tpu,Er,Ep,Vo,T2u] = diasmetic_rotations(Tpm,T2o,F1,F2);
    ...
    %sxx = chol(pinv(cov(Er)),'lower');
    %syx = (Er'*T2u*Lobs);
    %Fp(ii) = norm(Er-T2u,'fro')^2;
    Fp(ii) = trace(Er'*T2u);
    %Fp(ii) = 2*trace(Er'*T2u) - norm(T2u,'fro')^2;
    %Fp(ii) = trace(Er'*T2u);
...
function [R,P,T1u,Er,Ep,T2ua,S1u,V1u,S2u,V2u] = diasmetic_rotations(T1o,T2o,F1,F2)

[T1u, ord1, ~] = uniquetol(T1o,1e-6, 'ByRows', true, 'PreserveRange', true);
[T2u, ord2, ~] = uniquetol(T2o,1e-6, 'ByRows', true, 'PreserveRange', true);

[U1u,S1u,V1u] = svds(T1u,size(T1o,2)); %re-orthogonalize. 
[U2u,S2u,V2u] = svds(T2u,size(T1o,2));
